...
//DB에 작성한 편지 SELECT
letterMng.prototype.selectLetter = (query) => {
    
    // 쿼리: MEMBER_LETTER 테이블에서 이전에 강아지에게 보냈던 편지에 대한 정보를 조회한다.
    function selectMemberLetters(query) {
        return {
    ...
    return new Promise((resolve, reject) => {
        mySQLQuery(selectMemberLetters(query)) 
        .then((res) => { 
...
letterMng.prototype.insertNewLetter = (query) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        .then((res) => { 
          logger.warn(`편지id : ${res.letter_id}`);
          return resolve(res.letter_id); // res.letter_id : insert한 row의 id를 얻는다.
        })
        .catch((err) => {
